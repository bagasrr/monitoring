...
const Card = ({ cardTitle = "Room 1", Temp, Hum, Press, lastUpdated }) => {
  ...
  return (
    <div className="bg-teal-500 w-[300px] p-5 drop-shadow-xl shadow-[0_0_10px_#08f,0_0_20px_#08f,0_0_30px_#08f,0_0_40px_#08f]">
      ...
      <div>
        <CardData name="Temp" value={parseFloat(Temp)} unit="°C" className={getColor(parseFloat(Temp), TempRule)} />
        <CardData name="Hum" value={parseFloat(Hum)} unit="%" className={getColor(parseFloat(Hum), HumRule)} />
        <CardData name="Press" value={parseFloat(Press)} unit="Pa" className={getColor(parseFloat(Press), PressRule)} />
      </div>
      <div className="mt-5 text-white text-xs">{lastUpdated}</div>
    </div>
  );
